...
class HashTable:
    ...
    def hash_function(self,key):
        """
        Hash function to map a key (word) to an index in the hash table.
        Uses the sum of ASCII values of characters in the key and takes modulo with table size.
      """
    ...
    def insert(self,key,value):
        ...
        Handles collisions by appending to the linked list at the hashed index.
        :param key: what key we want to insert
        :param value: what's its value
        :return: nothing just insertes the key and its value ot hash table
        """
    ...
    def delete(self,key):
        ...
        Removes the node with the specified key if it exists.
        :param key: whihc key we wwant to delete
        :return: delete that key value from the hash table
        """
        index = self.hash_function(key)
        previous = None
        current = self.table[index]
    ...
    def increase(self,key):
        """
        Increase the value (count) of a key in the hash table by 1.
        :param key: key which should be increased
        :return: increased value of that key inside the hash table
        """
        index = self.hash_function(key)
        head = self.table[index]
        current = head
        ...
        self.insert(key,1) # if the key is not found, insert it with a count of 1

    def find(self,key):
        """
        Find the value (count) of a key in the hash table.
    ...
    def list_all_keys(self):
        """
        :return:
        the function will Print all key-value pairs in the hash table.
        """
        outcomes = []
        for i in range(self.size):
            current = self.table[i]
            while current:
                outcomes.append((current.key, current.count))
                current = current.next
        return outcomes

    def export_from_file(self, filename):
        """
        Export all key-value pairs in the hash table to a file.
        """
        with open(filename, 'w') as file:
            for i in range(self.size):
                current = self.table[i]
                while current:
                    file.write(f'{current.key}: {current.count}\n')  # Write each key-value pair to the file
                    current = current.next

    def collision_histogram(self):
        """
        Create a histogram of collision list lengths.
        Returns a list where each element represents the length of the linked list at each index.
        """
        histogram = [0] * self.size  # Initialize histogram list
        for i in range(self.size):
            current = self.table[i]
            count = 0
            while current:
                count += 1
                current = current.next
            histogram[i] = count  # Store the length of the list in the histogram
        return histogram

    def variance_of_collisions(self):
        """
        Calculate the variance of collision list lengths.
        Measures how spread out the lengths of the collision lists are.
        """
        histogram = self.collision_histogram()  # Get the histogram of list lengths
        #lets take out the empty list
        non_empty_length = [x for x in histogram if x>0]
        if not non_empty_length:
            return 0
        avg = sum(histogram) / self.size  # Calculate the average length
        variance = sum((x - avg) ** 2 for x in histogram) / self.size  # Compute the variance
        return variance
...
subscribe to our email newsletter to hear about new eBooks."""
    ...
    # Print variance of collisions
    print("Variance of collisions:", hash_table.variance_of_collisions())

    # Export key-value pairs to a file
    #hash_table.export_from_file('output.txt')

    data = hash_table.collision_histogram()
    #print(data,"Data")
    plt.figure()
    plt.bar(range(len(data)), data)
    plt.title('Collision List Lengths')
    plt.ylabel('Frequency')
    plt.xlabel('Length of collision list')
    plt.show()

    """
    text_1 = " The Long Text data type stores up to 65,536 alphanumeric characters and supports rich text formatting, such as different colors, fonts, and highlighting."
    words = text_1.split()
    sizes =[30,300,1000]
    for size in sizes:
        print(f"\n Testing with hash table with size: {size}")

        hash_table = HashTable(size)
        #inserting words ito hashtable
        for word in words:
            hash_table.increase(word)

        find_word = "stores"
        print(f"Count for {find_word}:",hash_table.find(find_word))



        #delete the word if exists
        hash_table.delete(find_word)
        print(f"Count for {find_word} after deletion", hash_table.find(find_word))


        #increase
        hash_table.increase(find_word)
        print(f"NEW Count for {find_word}:",hash_table.find(find_word))


        #let's  list all key value pairs
        print("All key value pairs")
        for key,count in hash_table.list_all_keys():
            print(f"{key}: {count}")

        
       
        #cal variance
        var = hash_table.variance_of_collisions()
        print("Variance: {var}")
